...
(function() {
  var SOCKET_TEMPLATE, createServer, ejs, err, f5DeleteFile, fs, getTempl, http, insertSocket, insertTempl, io, path, renderDir, res500, sortAlphaNum, sortFiles, types, url, watcher;
  ...
  insertTempl = function(file, templ) {
    ...
    if (!file.search(matchrx)) {
      ...
      return file.replace(matchrx, function(match, offset, str) {
        return "" + (templ.join('\n')) + "\n" + match;
      });
    }
  };
  ...
  createServer = function(config) {
    var change, server, sockets, _fn, _i, _io, _len, _path, _port, _ref, _ref1, _sockets;

    _path = config.path;
    _port = config.port;
    server = http.createServer(function(req, res) {
      var pathname, realPath, renderPath;
    ...
    sockets.on("connection", function(socket) {
      ...
      return socket.on("rename", function(data) {
        return f5Rename(data);
      });
    });
    _ref1 = ["fileCreated", "fileModified", "fileDeleted"];
    _fn = function(chg) {
      return watcher.on(chg, function(file) {
